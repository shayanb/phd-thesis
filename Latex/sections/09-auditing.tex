...
In Ethereum, there are two types of addresses: Externally Owned Accounts (EOA) and smart contracts. An EOA is the equivalent of an address in Bitcoin and has a private key associated with it. The address value itself is based on a hash of the user's public key. The address can be used to send a transaction, which is signed by the EOA's associated private key. 
